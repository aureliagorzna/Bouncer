...
const boom = (x, y) => {
    for (let arenaY = y - Math.floor(explosionRange); arenaY < y + Math.floor(explosionRange); arenaY++) {
        for (let arenaX = x - Math.floor(explosionRange); arenaX < x + Math.floor(explosionRange); arenaX++) {
            const expression1 = Math.pow(arenaY - y, 2);
            const expression2 = Math.pow(arenaX - x, 2);
            const distance = Math.sqrt(expression1 + expression2);
            if (distance < Math.floor(explosionRange) && arena[arenaY] && arena[arenaY][arenaX] != null)
